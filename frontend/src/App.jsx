...
function Navbar({ onLogin, user, onLogout }) {
    ...
    <nav style={{
      ...
      <Link to="/" style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '10px' }}>
        <div style={{
          ...
          fontSize: 18, fontWeight: 800, color: 'white', fontFamily: 'Space Grotesk',
          flexShrink: 0, boxShadow: '0 0 16px rgba(124,58,237,0.5)'
        }}>P</div>
        <span style={{ fontFamily: 'Space Grotesk', fontWeight: 700, fontSize: 20, color: '#fff', whiteSpace: 'nowrap' }}>
        ...
        {user ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{
              ...
              <div>
                <p style={{ fontSize: 12, fontWeight: 600, color: '#e2e8f0', lineHeight: 1, margin: 0 }}>{user.name}</p>
                <p style={{ fontSize: 10, color: '#64748b', marginTop: 2, margin: 0 }}>
                  {user.role === 'govt' ? '🏛 Government' : '👤 Citizen'}
                </p>
              </div>
            </div>
            <button
      ...
      <div style={{
        ...
        <div style={{ marginBottom: 28 }}>
          ...
          <h2 style={{
            ...
            fontWeight: 800, color: '#f1f5f9', marginBottom: 6
          }}>
            {isGovt ? 'Official Login' : 'Welcome back'}
          </h2>
          <p style={{ fontSize: 13, color: '#475569', margin: 0 }}>
            {isGovt
              ? 'Access the government dashboard and citizen reports'
              : 'Get personalised city alerts and live updates'}
          </p>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <button
            onClick={() => onSuccess({ name: isGovt ? 'Officer Kumar' : 'Citizen', role: type })}
            style={{
          ...
          <p style={{ fontSize: 12, color: '#334155', textAlign: 'center', margin: 0 }}>
            {isGovt
              ? 'For official use only. Unauthorised access is prohibited.'
...
function AppInner() {
  ...
  return (
    <div style={{ paddingTop: 64 }}>
      <Navbar onLogin={setLoginType} user={user} onLogout={() => setUser(null)} />
      {loginType && (
        <LoginModal
          type={loginType}
